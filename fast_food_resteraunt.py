# Luke Psyhogios, April 17th 2022, Section 001, Assignment 7, Problem #2: Fast Food Restaurant
...
while valid_1 == True:
    ...
    if action == 's':
        ...
        if user_selection in product_names:
            ...
            print('We currently have', product_quantities[product_index], 'in stock')
            print('\n')
        else:
            # Display output
        ...
        # Get the product the user would like to update
            ...
            if update_action == 'n':
                while valid_5 == True:
                    ...
                    if new_product in product_names:
                        # If the user enters a product that already exists, print below and continue the loop
        ...
        # Set variables and lists for calculations
        total_cost_list = []
        high_cost = max(product_costs)
        low_cost = min(product_costs)
        # Find the location of the high and low prices
        ...
        for i in range(0, len(product_costs)):
            # Multiply every item by its quantity
        ...
        # If user does not enter a valid action
